  ...
  //gibt false zurück, wenn Benutzer bereits vorhanden ist
    public boolean insertMovieUser() throws SQLException {
      ...
      while (rs.next()) {
        ...
        //Benutzer eintragen
      DBC.executeQuery("INSERT INTO [MovieDB].[dbo].[movieUser] VALUES('" + movieUser.getUserName() + "', " + movieUser.getUserPassword() + ")");
      return true;
  }
    ...
    public int getUserIdByUserName() throws SQLException{
        ResultSet rs = DBC.getRS("SELECT userId FROM [MovieDB].[dbo].[movieUser] WHERE userName = '" + movieUser.getUserName() + "'");
        rs.next();
